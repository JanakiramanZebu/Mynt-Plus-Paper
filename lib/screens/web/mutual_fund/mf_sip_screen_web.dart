...
import 'mf_sip_details_screen_web.dart';
import '../../../sharedWidget/common_search_fields_web.dart';
import 'mf_cancel_sip_alert_web.dart';

class MFSipdetScreenWeb extends ConsumerStatefulWidget {
  const MFSipdetScreenWeb({super.key});

  @override
  ConsumerState<MFSipdetScreenWeb> createState() => _MFSipdetScreenState();
}

class _MFSipdetScreenState extends ConsumerState<MFSipdetScreenWeb>
  ...
  // Hover state for Active SIPs table
  ...
  void _onHoverChanged() {
    final currentHoveredRow = _hoveredRowIndex.value;
    if (currentHoveredRow == null && _popoverRowIndex != null && !_isHoveringDropdown) {
  ...
  // Helper to check if SIP actions should be shown
  bool _shouldShowSipActions(dynamic item) {
                              ...
                              rows: [
                                ...sortedOrders.asMap().entries.map((entry) {
                                  ...
                                  return shadcn.TableRow(
                                    cells: [
                                      // Scheme with hover dropdown button
                                      _buildCellWithHover(
                                        rowIndex: rowIndex,
                                        columnIndex: 0,
                                        isOrderBook: false,
                                          ...
                                          builder: (context, hoveredIndex, _) {
                                            ...
                                            return Stack(
                                              ...
                                              children: [
                                                ...
                                                if (isRowHovered && _shouldShowSipActions(item))
                                                  Positioned(
                                                    right: 0,
                                                    top: 0,
                                                    bottom: 0,
                                                    child: Align(
                                                      ...
                                                      child: _buildOptionsMenuButton(item, rowIndex, mfData),
                                                    ),
                                                  ),
                                              ],
                                            );
                                ...
                                ...sortedOrders.asMap().entries.map((entry) {
                                  ...
                                  return shadcn.TableRow(
                                    cells: [
                                      ...
                                      // Fund name with hover dropdown button
                                      _buildCellWithHover(
                                        rowIndex: rowIndex,
                                        columnIndex: 4,
                                        isOrderBook: true,
                                              ...
                                              children: [
                                                ...
                                                if (isRowHovered)
                                                  Positioned(
                                                    ...
                                                    child: Align(
                                                      alignment: Alignment.centerRight,
                                                      child: _buildOrderBookOptionsMenuButton(item, rowIndex, mfData),
                                                    ),
                                                  ),
                                              ],
                                            );
  ...
  shadcn.TableCell _buildCellWithHover({
    ...
    return shadcn.TableCell(
      ...
      child: MouseRegion(
        ...
        child: ValueListenableBuilder<int?>(
          valueListenable: _hoveredRowIndex,
          child: child, // Cache the child widget to prevent unnecessary rebuilds
          builder: (context, hoveredRow, cachedChild) {
            // Row is hovered if mouse is over it OR if its dropdown menu is open
            final isHovered = hoveredRow == rowIndex ||
                (_activePopoverController != null && _popoverRowIndex == rowIndex);
  ...
  Widget _buildOptionsMenuButton(dynamic item, int rowIndex, MFProvider mfData) {
    final showActions = _shouldShowSipActions(item);
      ...
      builder: (buttonContext) {
        return GestureDetector(
          onTap: () {
            ...
            // Details option (always available)
            menuItems.add(
              _buildMenuButton(
                ...
                onPressed: (ctx) {
                  _closePopover();
                  _showSipDetail(mfData, item, context);
                },
              ),
            );
  ...
  // Order Book options menu (Details only)
  Widget _buildOrderBookOptionsMenuButton(dynamic item, int rowIndex, MFProvider mfData) {
    return Builder(
      builder: (buttonContext) {
        return GestureDetector(
          onTap: () {
            ...
            // Build menu items - only Details for Order Book
            List<shadcn.MenuItem> menuItems = [];
            final iconColor = resolveThemeColor(context,
                dark: MyntColors.textPrimaryDark,
                light: MyntColors.textPrimary);
              ...
              return Align(
                ...
                child: Material(
                  ...
                  child: Container(
                    ...
                    child: mfSipdetScrenWeb(data: item),
                  ),
                ),
              );
            },
