...
class _SecureFundWebState extends ConsumerState<SecureFundWeb> {
  ...
  void _openAddMoney() {
    if (isPaperTrading) {
      _showPaperWalletDialog(isDeposit: true);
      return;
    }
  ...
  void _openWithdraw() async {
    if (isPaperTrading) {
      _showPaperWalletDialog(isDeposit: false);
      return;
    }
    final trancation = ref.read(transcationProvider);
    // Ensure bank details and client data are fetched
  ...
  void _showPaperWalletDialog({required bool isDeposit}) {
    final amountController = TextEditingController();
    final wallet = VirtualWallet.instance;
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Amount input
                    Text(
                      isDeposit ? 'Enter amount to add' : 'Enter amount to withdraw',
                      style: MyntWebTextStyles.bodySmall(
                        context,
                        color: resolveThemeColor(
                          context,
                    ...
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: [1000, 5000, 10000, 50000, 100000].map((amount) {
                        return InkWell(
                          onTap: () {
                          ...
                          child: Container(
                            ...
                            child: Text(
                              '\u20B9${amount >= 100000 ? '${amount ~/ 100000}L' : amount >= 1000 ? '${amount ~/ 1000}K' : amount.toString()}',
                              style: MyntWebTextStyles.bodySmall(
                                context,
                                color: resolveThemeColor(
                                  context,
                    ...
                    SizedBox(
                      ...
                      child: MyntPrimaryButton(
                        label: isDeposit ? 'Add Money' : 'Withdraw',
                        isFullWidth: true,
                        ...
                        onPressed: () async {
                          ...
                          if (amount <= 0) {
                            showResponsiveWarningMessage(context, 'Enter a valid amount');
                            return;
                          }

                          if (isDeposit) {
                            await wallet.credit(amount, 'Manual deposit');
                          } else {
                            ...
                            await wallet.debit(amount, 'Manual withdrawal');
                          }

                          // Refresh fund display
                          if (mounted) {
                            ref.read(fundProvider).fetchFunds(this.context);
                          }
                          ...
                          showResponsiveSuccess(
                            ...
                            isDeposit
                                ...
                                : '\u20B9${amount.toStringAsFixed(2)} withdrawn successfully',
                          );
                        },
                      ),
                    ),
