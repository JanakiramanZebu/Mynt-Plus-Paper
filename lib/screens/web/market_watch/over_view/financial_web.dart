...
import 'chart_web.dart';

class FinancialWidgetWeb extends ConsumerWidget {
  const FinancialWidgetWeb({super.key});
...
// unique income chart
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  border: Border.all(color: MyntColors.textSecondary.withOpacity(0.1)),
                                      ...
                                      child: Container(
                                        ...
                                        child: Text(
                                          ...
                                          style: MyntWebTextStyles.para(
                                            ...
                                            fontWeight: isSelected ? MyntFonts.bold : MyntFonts.regular,
                                          ),
                                        ),
                                      ),
                                    );
...
class BalanceSheetDataWeb extends StatelessWidget {
  final List<BalanceSheet> balanceSheet;
  final String financialYear;
  final ThemesProvider themes;
  const BalanceSheetDataWeb(
...
class BalanceSheetTableWeb extends StatelessWidget {
  final List<BalanceSheet> data;
  final ThemesProvider themes;

  const BalanceSheetTableWeb({
    super.key,
    required this.data,
    ...
    return Container(
      child: Column(
        ...
        children: [
          ...
          _buildHeaderRow(context, sortedData),
          // Data rows with names
          ..._buildMetricRows(context, sortedData),
          // Legend below table
  ...
  Widget _buildHeaderRow(BuildContext context, List<BalanceSheet> sortedData) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          ...sortedData
              .map((item) => Expanded(
                    child: Text(
                      _formatDate(item.yearEndDate ?? item.convDate),
  ...
  List<Widget> _buildMetricRows(BuildContext context, List<BalanceSheet> sortedData) {
    final metrics = [
      ...
      child: Row(
        children: [
          ...
          ...sortedData.map((item) => Expanded(
            child: Text(
              _formatValue((metric["getValue"] as Function)(item)),
              style: MyntWebTextStyles.body(
                context,
                fontWeight: MyntFonts.regular,
                color : resolveThemeColor(
...
class IncomeSheetDataWeb extends StatelessWidget {
  final List<IncomeSheet> incomSheet;
  final String financialYear;
  final ThemesProvider themes;
  const IncomeSheetDataWeb(
      {super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return IncomeStatementTableWeb(
      data: recentData,
      themes: themes,
    );
  }
...
class IncomeStatementTableWeb extends StatelessWidget {
  final List<IncomeSheet> data;
  final ThemesProvider themes;

  const IncomeStatementTableWeb({
    super.key,
    required this.data,
    required this.themes,
  });
  ...
  Widget _buildHeaderRow(BuildContext context, List<IncomeSheet> sortedData) {
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 0),
      child: Row(
        children: [
  ...
  List<Widget> _buildMetricRows(BuildContext context, List<IncomeSheet> sortedData) {
    final metrics = [
...
class CashFlowSheetDataWeb extends StatelessWidget {
  final List<CashflowSheet> cashFlowSheet;
  final String financialYear;
  final ThemesProvider themes;
  const CashFlowSheetDataWeb(
...
class CashFlowTableWeb extends StatelessWidget {
  final List<CashflowSheet> data;
  final ThemesProvider themes;

  const CashFlowTableWeb({
    super.key,
    required this.data,
    required this.themes,
  });
  ...
  Widget _buildHeaderRow(BuildContext context, List<CashflowSheet> sortedData) {
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 0),
      child: Row(
        children: [
  ...
  List<Widget> _buildMetricRows(BuildContext context, List<CashflowSheet> sortedData) {
    final metrics = [
