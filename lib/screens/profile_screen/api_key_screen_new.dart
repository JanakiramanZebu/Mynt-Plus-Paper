...
class _ApiKeyScreenNewState extends ConsumerState<ApiKeyScreenNew> {
  ...
  Future<void> _submitForm() async {
    ...
    if (provider.urlController.text.trim().isEmpty) {
      setState(() {
      ...
      final uri = Uri.tryParse(provider.urlController.text);
    ...
    if (provider.primaryIpController.text.trim().isEmpty) {
      setState(() {
      ...
      if (!ipRegex.hasMatch(provider.primaryIpController.text.trim())) {
        setState(() {
    ...
    // Validate Backup IP Address (optional but must be valid format if provided)
    if (provider.backupIpController.text.trim().isNotEmpty) {
      final ipRegex = RegExp(r'^(\d{1,3}\.){3}\d{1,3}$');
      if (!ipRegex.hasMatch(provider.backupIpController.text.trim())) {
        setState(() {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      CustomTextFormField(
                        ...
                        onChanged: (value) {
                          setState(() {
                            _errorMessageUrl = null; // Clear error when user types
                          });
                        },
                      ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                CustomTextFormField(
                                  ...
                                  onChanged: (value) {
                                    setState(() {
                                      _errorMessageBackupIp = null; // Clear error when user types
                                    });
                                  },
                                ),
