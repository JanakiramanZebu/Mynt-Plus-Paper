...
class _ModifyPlaceOrderScreenState extends ConsumerState<ModifyPlaceOrderScreen> {
  ...
  void initState() {
    ...
    setState(() {
      ...
      // ];
      addValidity =
          validity.toUpperCase() == 'IOC' || (widget.modifyOrderArgs.dscqty != null && int.parse(widget.modifyOrderArgs.dscqty.toString()) > 0) ? true : false;
                  ...
                  children: [
                    SingleChildScrollView(
                        ...
                        child: Column(mainAxisSize: MainAxisSize.max, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    ...
                                    children: [
                                      ...
                                      SizedBox(
                                        ...
                                        child: CustomTextFormField(
                                            ...
                                            onChanged: (value) {
                                              ...
                                              if (value.isNotEmpty && inputPrice > 0) {
                                                ...
                                                if (!regex.hasMatch(value)) {
                                                  ...
                                                  priceCtrl.selection = TextSelection.collapsed(offset: priceCtrl.text.length); // Keep cursor at the end
                                                }
                                              }
                                              ScaffoldMessenger.of(context).removeCurrentSnackBar();
                                              if (value.isEmpty || inputPrice <= 0) {
                                                ScaffoldMessenger.of(context)
                                                    .showSnackBar(warningMessage(context, "Limit Price can not be ${inputPrice <= 0 ? 'zero' : 'empty'}"));
                                              } else {
                                            ...
                                            hintStyle: textStyle(theme.isDarkMode ? colors.colorWhite : colors.colorBlack, 15, FontWeight.w400),
                                            keyboardType: const TextInputType.numberWithOptions(decimal: true),
                                            style: textStyle(const Color(0xff000000), 16, FontWeight.w600),
                                            isReadable: prcType == "MKT" || prcType == "SL-MKT" ? true : false,
                          ...
                          if (_isBOCOOrderEnabled) ...[const SizedBox(height: 16), stopLossOption(theme, context, widget.scripInfo)],
                          // Padding(
                          //     padding:
                              ...
                              child: ElevatedButton(
                                  onPressed: internet.connectionStatus == ConnectivityResult.none
                                      ...
                                      : () async {
                                          if (!orderProvide.orderloader) {
                                            ...
                                            } else if (qtyCtrl.text == "0" || priceCtrl.text == "0") {
                                              ScaffoldMessenger.of(context)
                                                  .showSnackBar(warningMessage(context, qtyCtrl.text == "0" ? "Quantity can not be 0" : "Price can not be 0"));
                                            } else if ((double.parse(prcType == "MKT" || prcType == "SL-MKT" ? price : priceCtrl.text) <
                                            ...
                                            } else if ((prcType == "SL-LMT" || prcType == "SL-MKT")) {
                                              if (triggerPriceCtrl.text.isEmpty || triggerPriceCtrl.text == "0") {
                                                ...
                                                if (isBuy) {
                                                  if (prcType == "SL-LMT") {
                                                    ...
                                                    } else if (double.parse(priceCtrl.text) < double.parse(triggerPriceCtrl.text)) {
                                                      ScaffoldMessenger.of(context).showSnackBar(warningMessage(context, "Trigger should be less than price"));
                                                    } else if (double.parse(triggerPriceCtrl.text) > double.parse(widget.scripInfo.uc ?? "0.00")) {
                                                      ScaffoldMessenger.of(context).showSnackBar(warningMessage(
                                                          context, "Trigger can not be greater than upper circuit limit of ${widget.scripInfo.uc ?? 0.00}"));
                                                    } else {
                                                  ...
                                                  if (prcType == "SL-LMT") {
                                                    if (double.parse(triggerPriceCtrl.text) > double.parse(widget.orderArg.ltp ?? "0.00")) {
                                                      ScaffoldMessenger.of(context).showSnackBar(warningMessage(context, "Trigger should be lesser than LTP"));
                                                    ...
                                                    if (double.parse(triggerPriceCtrl.text) > double.parse(widget.scripInfo.uc ?? "0.00")) {
                                                      ScaffoldMessenger.of(context).showSnackBar(warningMessage(
                                                          context, "Trigger can not be greater than upper circuit limit of ${widget.scripInfo.uc ?? 0.00}"));
                                                    } else if (double.parse(price) > double.parse(triggerPriceCtrl.text)) {
                                                      ScaffoldMessenger.of(context)
                                  ...
                                  child: orderProvide.orderloader
                                      ? SizedBox(
                                          width: 18,
                                          height: 20,
                                          child: CircularProgressIndicator(strokeWidth: 2, color: theme.isDarkMode ? colors.colorBlack : colors.colorWhite),
                                        )
                                      : Text("Modify Order", style: textStyle(theme.isDarkMode ? colors.colorBlack : colors.colorWhite, 14, FontWeight.w600)))),
          ...
          SizedBox(
              ...
              child: CustomTextFormField(
                  ...
                  onChanged: (value) {
                    ...
                    if (value.isEmpty || inputPrice <= 0) {
                      ScaffoldMessenger.of(context).hideCurrentSnackBar();
                      ScaffoldMessenger.of(context).showSnackBar(warningMessage(context, "Trigger can not be ${inputPrice <= 0 ? 'zero' : 'empty'}"));
                    }
                  },
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  style: textStyle(theme.isDarkMode ? colors.colorWhite : colors.colorBlack, 16, FontWeight.w600),
            ...
            SizedBox(
                ...
                child: CustomTextFormField(
                    ...
                    onChanged: (value) {
                      ...
                      ScaffoldMessenger.of(context).hideCurrentSnackBar();
                      if (value.isEmpty || inputPrice <= 0) {
                        ScaffoldMessenger.of(context).showSnackBar(warningMessage(context, "Target can not be ${inputPrice <= 0 ? 'zero' : 'empty'}"));
                      }
                    },
                    hintStyle: textStyle(const Color(0xff666666), 15, FontWeight.w400),
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
            ...
            SizedBox(
                ...
                child: CustomTextFormField(
                    ...
                    onChanged: (value) {
                      ...
                      ScaffoldMessenger.of(context).hideCurrentSnackBar();
                      if (value.isEmpty || inputPrice <= 0) {
                        ScaffoldMessenger.of(context).showSnackBar(warningMessage(context, "Stoploss can not be ${inputPrice <= 0 ? 'zero' : 'empty'}"));
                      }
                    },
                    hintText: "0.00",
                    hintStyle: textStyle(const Color(0xff666666), 15, FontWeight.w400),
  ...
  modifyOrder() async {
    ...
    if (placeorder) {
      ...
      ModifyOrderInput input = ModifyOrderInput(
          ...
          qty: widget.modifyOrderArgs.exch == 'MCX' ? (int.parse(qtyCtrl.text) * lotSize).toString() : qtyCtrl.text,
          ret: validity,
