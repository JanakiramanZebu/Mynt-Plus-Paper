...
class _OrderBookState extends State<OrderBook> {
  ...
  void _processUpdates(Map socketDatas) {
    ...
    for (var order in items) {
      ...
      if (lp != null && lp != "null" && lp != "0" && lp != "0.00" && lp != currentLtp) {
        order.ltp = lp;
        hasUpdates = true;
      }
      ...
      if (pc != null && pc != "null" && pc != "0" && pc != "0.00" && pc != currentPerChange) {
        order.perChange = pc;
        hasUpdates = true;
      }
            ...
            child: RefreshIndicator(
              ...
              child: searchorder!.isEmpty
                ? widget.orderBook.isNotEmpty
                  ? _buildOrderList(widget.orderBook, order, theme, watch)
                  : const SizedBox(height: 500, child: NoDataFound())
                : searchorder.isNotEmpty
                  ? _buildOrderList(searchorder, order, theme, watch)
                  : Container()
            ),
          )
        ]),
  ...
  // Order list view
  Widget _buildOrderList(List<OrderBookModel> items, OrderProvider order, ThemesProvider theme, ScopedReader watch) {
        ...
        return Builder(
          builder: (itemContext) {
            return _OrderItem(
              ...
              theme: theme,
              onTap: () => _navigateToOrderDetails(items[itemIndex], watch, itemContext),
              onLongPress: () {
                if (order.openOrder!.isNotEmpty && 
                    (order.tabCtrl.index == 1 || 
                     !["COMPLETE", "CANCELED", "REJECTED"].contains(items[itemIndex].status))) {
                  Navigator.pushNamed(itemContext, Routes.orderExit, arguments: order.openOrder);
                }
              },
            );
          }
  ...
  void _navigateToOrderDetails(OrderBookModel order, ScopedReader watch, BuildContext context) async {
    try {
      ...
      final quoteResponse = await watch(marketWatchProvider).fetchScripQuote(
        "${order.token}",
        "${order.exch}",
        context
      );
      ...
      if (quoteResponse != null && quoteResponse.stat == "Ok") {
        final getQuotes = context.read(marketWatchProvider).getQuotes;
        if (getQuotes != null) {
          // Update LTP if available
          if (getQuotes.lp != null && getQuotes.lp != "0" && getQuotes.lp != "0.00") {
            order.ltp = getQuotes.lp;
          }
          
          // Update close price if available
          if (getQuotes.c != null && getQuotes.c != "0" && getQuotes.c != "0.00") {
            order.c = getQuotes.c;
          ...
          if (ltp > 0 && closePrice > 0) {
            ...
            if (order.change == null || order.change == "null" || order.change == "0") {
              order.change = (ltp - closePrice).toStringAsFixed(2);
            }
            
            // Calculate percent change if missing
            if (order.perChange == null || order.perChange == "null" || order.perChange == "0") {
              order.perChange = ((ltp - closePrice) * 100 / closePrice).toStringAsFixed(2);
            }
          }
        }
      }
      ...
      // Fetch order history
      context.read(orderProvider).fetchOrderHistory("${order.norenordno}", context);
      
      // Fetch tech data for equity instruments
      if ((order.exch == "NSE" || order.exch == "BSE") && (order.instname.toString() != "UNDIND")) {
        context.read(marketWatchProvider).depthBtns.add({
          "btnName": "Fundamental",
          "imgPath": assets.dInfo,
          "key": context.read(showcaseProvide).fundamentalcase,
  ...
  String _getValidPrice(OrderBookModel order) {
    ...
    if (order.ltp != null && order.ltp != "null" && order.ltp != "0" && order.ltp != "0.00") {
      ...
      return order.prc!;
    } else if (order.c != null && order.c != "null" && order.c != "0" && order.c != "0.00") {
      return order.c!;
    } else if (order.close != null && order.close != "null" && order.close != "0" && order.close != "0.00") {
      return order.close!;
    }
    return "0.00";
  }
...
class _OrderItemState extends State<_OrderItem> {
  ...
  Widget _buildExchangeRow(ThemesProvider theme) {
    return Row(
      ...
      children: [
        ...
        RepaintBoundary(
          child: Text(
            " (${widget.orderItem.perChange ?? 0.00}%)",
            style: textStyle(
              _getPercentChangeColor(),
              12,
              FontWeight.w500
            )
          ),
  ...
  Widget _buildQuantityRow(ThemesProvider theme) {
    ...
    final filledQty = widget.orderItem.status != "COMPLETE" && (widget.orderItem.fillshares?.isNotEmpty ?? false) 
    ...
    final displayTotalQty = ((int.tryParse(widget.orderItem.qty.toString()) ?? 0) / lotSize).toInt();
      ...
      children: [
        Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                color: theme.isDarkMode
                  ...
                  : Color(widget.orderItem.trantype == "S" ? 0xffFCF3F3 : 0xffECF8F1)
              ),
              child: Text(
                widget.orderItem.trantype == "S" ? "SELL" : "BUY",
                style: textStyle(
                  widget.orderItem.trantype == "S" ? colors.darkred : colors.ltpgreen,
                  12,
                  FontWeight.w600
                )
              )
            ...
            Text(
              formatDateTime(value: widget.orderItem.norentm!).substring(13, 21),
              style: textStyle(
                const Color(0xff666666),
                12,
                FontWeight.w500
      ...
      children: [
          ...
          children: [
            SvgPicture.asset(
              widget.orderItem.status == "COMPLETE"
                ...
                : widget.orderItem.status == "CANCELED" || widget.orderItem.status == "REJECTED"
        ...
        Row(
          children: [
            ...
            Text(
              "${widget.orderItem.avgprc ?? widget.orderItem.prc ?? 0.00}",
              style: textStyle(
                theme.isDarkMode ? colors.colorWhite : colors.colorBlack,
                14,
                FontWeight.w500
            ...
            if (widget.orderItem.prctyp == "SL-LMT" || widget.orderItem.prctyp == "SL-MKT") ...[
  ...
  String _getValidPrice() {
    // First try LTP, then avgprc, then prc, then close, then default to "0.00"
    if (widget.orderItem.ltp != null && widget.orderItem.ltp != "null" && widget.orderItem.ltp != "0" && widget.orderItem.ltp != "0.00") {
      return widget.orderItem.ltp!;
    } else if (widget.orderItem.avgprc != null && widget.orderItem.avgprc != "null" && widget.orderItem.avgprc != "0" && widget.orderItem.avgprc != "0.00") {
      return widget.orderItem.avgprc!;
    } else if (widget.orderItem.prc != null && widget.orderItem.prc != "null" && widget.orderItem.prc != "0" && widget.orderItem.prc != "0.00") {
      return widget.orderItem.prc!;
    } else if (widget.orderItem.c != null && widget.orderItem.c != "null" && widget.orderItem.c != "0" && widget.orderItem.c != "0.00") {
      return widget.orderItem.c!;
    } else if (widget.orderItem.close != null && widget.orderItem.close != "null" && widget.orderItem.close != "0" && widget.orderItem.close != "0.00") {
