...
class _SliverTabsDelegate extends SliverPersistentHeaderDelegate {
  ...
  bool shouldRebuild(covariant SliverPersistentHeaderDelegate oldDelegate) {
    ...
    if (oldDelegate is _SliverTabsDelegate) {
      ...
      bool selectedTabChanged = selectedTabIndex != oldDelegate.selectedTabIndex;
      ...
      if (watchlistNames != null && oldDelegate.watchlistNames != null) {
        ...
        watchlistDataChanged = watchlistNames!.length != oldDelegate.watchlistNames!.length ||
...
class _WatchListScreen extends State<WatchListScreen> with AutomaticKeepAliveClientMixin {
  ...
  int _selectedTabIndex = 0; // Track selected tab index locally for immediate UI updates
  int _lastWatchlistCount = 0; // Track watchlist count to detect CRUD operations
  ...
  void _ensurePredefinedWatchlistsLoaded() async {
    ...
    try {
      ...
      print("Ensuring predefined watchlists are loaded. Current watchlist: $currentWatchlist");
      ...
      if (isCurrentPredefined) {
        ...
        if (marketWatch.scrips.isEmpty) {
          print("Current predefined watchlist has no data, fetching data for: $currentWatchlist");
        ...
        if (listName != currentWatchlist) {
          ...
          if (!marketWatch.marketWatchScripData.containsKey(listName) || 
              (marketWatch.marketWatchScripData.containsKey(listName) && 
               jsonDecode(marketWatch.marketWatchScripData[listName]).isEmpty)) {
  ...
  void _scrollToSelectedTab(int index, {bool force = false}) {
    ...
    if (!force && DateTime.now().difference(_lastUserScrollTime).inSeconds < 1) return;
    ...
    final double targetOffset = (index * tabWidth) - (viewportWidth / 2) + (tabWidth / 2);
  ...
  void _highlightActiveTab(WidgetRef ref, String wlName) {
    final marketWatchlist = ref.read(marketWatchProvider.select((p) => p.marketWatchlist));
      ...
      if (selectedIndex != -1) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!_isDisposed && _tabScrollController.hasClients) {
            _scrollToSelectedTab(selectedIndex); // Don't force, respect user scrolling
            
            // Force refresh for predefined watchlists when they become active
            if (wlName == "Nifty50" || wlName == "Niftybank" || wlName == "Sensex") {
              ...
              if (scrips.isEmpty) {
                ...
                marketWatch.fetchMWScrip(wlName, context).then((_) {
                  ...
                  marketWatch.requestMWScrip(context: context, isSubscribe: true);
                });
              }
            }
          }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer(builder: (context, WidgetRef ref, _) {
      ...
      final scripsLength = ref.watch(marketWatchProvider.select((p) => p.scrips.length));
...
final theme = ref.watch(themeProvider);
        ...
        if (savedIndex >= 0 &&
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (!_isDisposed && _controller.hasClients) {
              ...
              if (currentName != null && (currentName == "Nifty50" || 
                  currentName == "Niftybank" || 
                  currentName == "Sensex")) {
                // Immediately force refresh for predefined watchlists
                ref.read(marketWatchProvider).fetchMWScrip(currentName, context).then((_) {
      ...
      if (marketWatchlist?.values != null) {
        ...
        watchlistCountChanged = _lastWatchlistCount != 0 && _lastWatchlistCount != watchlistCount;
        ...
        if (watchlistCountChanged && !isPredefinedAddition) {
          print("DEBUG UI: Real CRUD operation detected, refreshing watchlist data...");
          WidgetsBinding.instance.addPostFrameCallback((_) async {
            if (!_isDisposed) {
              try {
                final marketWatch = ref.read(marketWatchProvider);
                print("Watchlist count changed, refreshing complete watchlist data...");
          ...
          print("DEBUG UI: Predefined lists addition detected, no additional refresh needed");
        }
      ...
      if (!_isUserScrolling && DateTime.now().difference(_lastUserScrollTime).inSeconds >= 1) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!_isDisposed) {
...
SvgPicture.asset(
                              ...
                              child: InkWell(
                                ...
                                onTap: () async {
                                  ...
                                  showModalBottomSheet(
                                    ...
                                    context: context,
                                    builder: (context) => const ScripFilterBottomSheet(),
                                  );
                                },
                                child: Padding(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          // Watchlist tabs
                          Expanded(
                            child: _buildWatchlistTabs(ref, wlName, marketWatchlist),
                          ),
                        ],
                      ),
                    ),
            ...
            onPageChanged: (int d) async {
              ...
              if (newWatchlistName == "My Stocks" ||
                ...
                if (newWatchlistName == "Nifty50" || 
                  ...
                  bool hasCachedData = marketWatch.marketWatchScripData.containsKey(newWatchlistName);
                  if (!hasCachedData) {
                    print("No cached data for $newWatchlistName, forcing refresh");
                    await marketWatch.fetchPreDefMWScrip(context);
                  }
                }
              } else {
  ...
  Widget _buildWatchlistTabs(WidgetRef ref, String wlName, dynamic marketWatchlist) {
    final theme = ref.watch(themeProvider);
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (isSelected) {
          print("Tab $index ($name) is selected, _selectedTabIndex: $_selectedTabIndex");
        }
          ...
          child: Material(
            ...
            child: InkWell(
              ...
              onTap: () async {
                ...
                if (name == "My Stocks" || name == "Nifty50" || name == "Niftybank" || name == "Sensex") {
...
,
  ...
  String _formatTabName(String name) {
    ...
    return name.length <= 10 ? "${name[0].toUpperCase()}${name.substring(1)}" : "${name.substring(0, 9)}..";
  }
