...
class _AddScripState extends ConsumerState<SearchScreen>
  ...
  void _scrollToSelectedTab(int index) {
    ...
    for (int i = 0; i < index; i++) {
      final String currentText = ref.read(marketWatchProvider).searchTabList[i].text ?? '';
             final bool isLongTab = (currentText == 'Currency' || currentText == 'Commodity') && tabCtrl.index == i;
       totalOffset += isLongTab ? 90.0 : tabWidth;
    }
    
    // Add half of the current tab's width
         final String currentTabText = ref.read(marketWatchProvider).searchTabList[index].text ?? '';
     final bool isCurrentLongTab = (currentTabText == 'Currency' || currentTabText == 'Commodity') && tabCtrl.index == index;
    final double currentTabWidth = isCurrentLongTab ? 90.0 : tabWidth;
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  const SizedBox(width: 8),
                                  Expanded(
                                    child: _buildSearchTabs(ref, theme, tabCtrl, tabcount, searchScrip),
                                  ),
                                ],
                              ),
                            ),
  ...
  Widget _buildSearchTabs(WidgetRef ref, theme, TabController tabCtrl, int tabcount, searchScrip) {
    final searchTabList = ref.read(marketWatchProvider).searchTabList.sublist(0, tabcount);
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // Dynamically adjust width for Currency and Commodity tabs when selected
                 final bool isLongTab = (tab.text == 'Currency' || tab.text == 'Commodity') && tabCtrl.index == index;
         final double dynamicWidth = isLongTab ? 90.0 : tabWidth;
                    ...
                    Container(
                      ...
                      child: Text(
                        ...
                        textAlign: TextAlign.center,
                                                  maxLines: 1,
                          overflow: tabCtrl.index == index ? TextOverflow.visible : TextOverflow.ellipsis,
                      ),
                    ),
